...
import 'select-players.page.dart';
...
class _AddMatchPageState extends State<AddMatchPage> {
  final List<int> playersIdsTeam1 = [
    MatchComponent.player1.id,
    MatchComponent.player2.id,
  ];
  final List<int> playersIdsTeam2 = [
  ...
  Widget _content(BuildContext context) {
    List<RankedPlayer> playersTeam1 =
        players.where((player) => playersIdsTeam1.contains(player.id)).toList();
    List<RankedPlayer> playersTeam2 =
        players.where((player) => playersIdsTeam2.contains(player.id)).toList();
        ...
        child: Form(
          child: Row(
            children: [
              ...
              Text(
                '-',
                style: $styles.text.h1.copyWith(color: $styles.colors.grey),
              ),
              SizedBox(width: $styles.insets.m),
              _scoreTextField(scoreStyle),
              const Spacer(),
  ...
  Widget _team(
    ...
    required List<int> selectedPlayers,
  }) {
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  for (RankedPlayer player in players)
                    _playerRow(player.name, player.rank),
                ],
              ),
            ),
          ],
